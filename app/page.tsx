    ...
    >
      ...
      <aside className="task-sidebar">
        <button
          ...
          onClick={() => setModal({ kind: "projects", title: "切换项目" })}
        >
          <div>
            <small>项目</small>
        ...
        <button
          className={`task-item task-button ${
            activeTask === "rna" ? "selected" : ""
          }`}
          onClick={() => selectTask("rna", task.title)}
        >
          <i className="dot yellow" />
          <div>
            <b>{task.title}</b>
            <small>
              {statusLabel[task.status] || task.status} · {task.progress}%
            </small>
          </div>
        </button>
        <button
          className={`task-item task-button ${
            activeTask === "literature" ? "selected" : ""
          }`}
          onClick={() => selectTask("literature", "文献证据图谱", "evidence")}
        >
        ...
        <button
          className={`task-item task-button ${
            activeTask === "structure" ? "selected" : ""
          }`}
          onClick={() => selectTask("structure", "蛋白质结构预览", "structure")}
        >
        ...
        {extraTasks.map((name, i) => (
          <button
            key={`${name}-${i}`}
            className={`task-item task-button ${
              activeTask === `extra-${i}` ? "selected" : ""
            }`}
            onClick={() => selectTask(`extra-${i}`, name)}
          >
            <i className="dot gray" />
            <div>
              <b>{name}</b>
        ...
        <div className="side-title">
          <span>数据集</span>
          <button
            onClick={() => setModal({ kind: "upload", title: "上传项目文件" })}
          >
            ＋ 上传
          </button>
        </div>
        <button
          ...
          onClick={() =>
            setModal({
              kind: "file",
              title: "counts.csv",
              detail: "RNA-seq 计数矩阵 · 2.4 MB · 24 个样本 · 18,432 个基因",
            })}
        >
