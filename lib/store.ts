...
const defaultState = (): State => ({
  task: {
    ...
    nodes: initialNodes.map((n: any) => ({
      ...n,
      status: n.id === "input" ? "succeeded" : "blocked",
      detail: n.id === "input"
...
function persist(s: State) {
  ...
  } catch { /* read-only deploys use process memory */ }
}
function state(): State {
  if (!g.__bioflow) {
    try {
      g.__bioflow = JSON.parse(fs.readFileSync(statePath, "utf8")) as State;
    } catch {
      g.__bioflow = defaultState();
    }
  }
  return g.__bioflow;
}
...
export function updateDemoConfig(input: Partial<DemoConfig>) {
  const s = state();
  s.config = normalizeDemoConfig({ ...configSnapshot(), ...input });
  s.task.title = s.config.title;
  s.task.goal = s.config.goal;
  s.task.nodes = s.task.nodes.map((node: any) =>
    node.id === "input"
      ? { ...node, detail: `counts.csv · ${s.config.sampleCount} 个样本` }
      : node
  );
  persist(s);
  return s.config;
...
export function createRun() {
  ...
  if (s.running) return { runId: "run_demo_001", status: "running" };
  s.running = true;
  s.cancelled = false;
  ...
  setTimeout(() => {
    if (s.cancelled) return;
    s.task.nodes = s.task.nodes.map((n: any) =>
      n.id === "design"
        ? { ...n, status: "running", detail: "正在校验元数据结构" }
        : n
    );
    persist(s);
    pushEvent(runId, "node.updated", {
      status: "running",
      detail: "正在校验元数据结构",
    }, "design");
    setTimeout(() => {
      if (s.cancelled) return;
      if (s.config.failAt === "none") {
        s.task.nodes = s.task.nodes.map((n: any) =>
          ["design", "de", "volcano", "report"].includes(n.id)
            ? {
              ...n,
              status: "succeeded",
              detail: n.id === "de"
                ? `已检验 ${s.config.geneCount.toLocaleString()} 个基因 · FDR < 0.05`
                : "结果已就绪",
            }
            : n
        );
      ...
      s.task.nodes = s.task.nodes.map((n: any) =>
        n.id === "design"
          ? { ...n, status: "failed", detail: "缺少 condition 字段" }
          : n
      );
...
export function retryNode(runId: string, nodeId: string) {
  ...
  s.task.nodes = s.task.nodes.map((n: any) =>
    n.id === nodeId
      ? { ...n, status: "succeeded", detail: "condition → 已完成映射" }
      : n.id === "de"
      ? { ...n, status: "running", detail: "正在运行 DESeq2" }
      : n
  );
  persist(s);
  ...
  setTimeout(() => {
    ...
    s.task.nodes = s.task.nodes.map((n: any) =>
      n.id === "de"
        ...
        : n.id === "volcano"
        ? { ...n, status: "succeeded", detail: "SVG + 交互式图表" }
        : n.id === "report"
        ? { ...n, status: "succeeded", detail: "Markdown 报告已就绪" }
        : n
    );
    s.task.progress = 100;
    s.task.status = "succeeded";
...
export function submitClarifications(answers: Record<string, string>) {
  const s = state();
  s.task.clarification = { status: "answered", answers };
  s.task.status = "awaiting_approval";
  persist(s);
